...
![Telegram workflow](/assets/og/post_telegram-workflow.png)
<span class="muted small">Fail, fail, fail, success, fail, fail...</span>
...
3. **GitHub Actions Workflows.**
  - **Workflow #1: Subscription Manager.** (`telegram-subscriptions.yml`) – Runs daily at 07:00 Pacific, and checks for `/subscribe` and `/unsubscribe` messages, adding/removing users from `_data/telegram_subscribers.json`, kicks out confirmation messages. 
  - **Workflow #2: Daily Song Sender** (`telegram-daily-song.yml`) – Runs a half hour later around 07:45 Pacific, reading today's song from YAML, sending to all subscribers in the JSON file (e.g., #325: "Can I Believe You" by Fleet Foxes [with clickable link to the track on YouTube Music])
...
Bottom line: I got from idea to fully functional Telegram bot with auto-subscriptions in one sub-100-message chat session—that there's some right-fine vibe codin'...at least for the standards of late 2025 (I'm sure it'll seem quaint in a year or two).
