...
description: "What it actually took to turn a personal afternoon project into a public Wear OS app."
---
...
But since there was no native Wear OS Swarm client, I suspected I wasn't the only one who'd be interested in this, so I went looking for testers in a couple of corners of Reddit (`r/WearOS/`, `r/SwarmApp/`, `r/Foursquare/`), and lo and behold, some latent demand from fellow Swarm nerds revealed itself...internationally, in fact: Australia, Deutschland, Greece, Nederland, España, et al. And that was all the confirmation I needed to turn this from a personal, sideloaded tool into something worth perfecting (i.e., get to an official public listing in the Google Play Store).

## The testers found the bugs co-pilot couldn't
...
Practically none of these were things an AI co-pilot could have caught on its own, and *a fortiori* for me: I could only catch things I noticed. For example, one user saw that sticker grid was left justified and running off the screen, but that's because he had a larger screen (later Pixel Watch model) which displayed it that way whereas on mine you couldn't tell.

## Reverse-engineering the parts Foursquare didn't document

The public Foursquare API (v2) is old, largely frozen, and it does more than its documentation admits. While building sticker support, I found that `checkins/add` quietly accepts a `stickerId` parameter that isn't documented anywhere—and it works, amazingly. It applies weekly account-level bonuses correctly. What it *doesn't* do is replicate the live, venue-specific multiplier the native Swarm app shows (the "3x right here, right now" bonuses)—that math happening behind an unexposed endpoint. Same for check-ins counting toward mayorships, I believe.

I originally displayed the static multiplier (e.g., "2x") next to each sticker. Then a tester checked in with a sticker showing 2x in Swarma and got 3x in the native app, on the same check-in, minutes apart. Because I couldn't get at Foursquare's private multiplier logic, the fix was to stop offering a guesstimate multiplier, and show a plain "this sticker has *a* bonus" glyph instead. Sometimes the right technical decision is to say less.
...
<p><a href="/swarma/" target="_blank"><img src="/assets/og/swarma/swarma_og.png" alt="Swarma landing page"></a><span class="muted small">Swarma v1.1, now available on the Play Store.</span></p>
